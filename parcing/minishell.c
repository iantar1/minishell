...
/*   Updated: 2023/05/10 20:28:53 by iantar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	modify_env_var(char	*var_name, char *new_val)
{
	...
	while (g_env)
	{
		if (!ft_strcmp(g_env->var_name, var_name) && var_name)
		{
			if (g_env->line)
				free(g_env->line);
			g_env->line = strdup_upgrade(new_val);
			g_env = tmp;
			return (g_env = tmp, 0);
		}
		g_env = g_env->next;
...
int	main(int ac, char *av[], char **env)
{
	...
	while (1)
	{
		...
		if (!line)
			return ((write(1, "\n", 1)), 0);
		add_history(line);
		line = strtrim_upgrade(line, "	 ");//you need to free?
		if (!line || !*line)
			continue ;
		pointer = line;
